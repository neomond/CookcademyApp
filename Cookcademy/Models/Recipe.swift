...
/// The enumeration conforms to CaseIterable.
/// This means that the enumeration Category can be iterated over to reference all of the possible cases.
/// Remember, since it conforms to CaseIterable, you can get all of the cases by using Unit.allcases.


/// The String(format:) method is used to create the formattedQuantity by returning a formatted String from a Double.
/// The %g String Format Specifier specifier suppresses any trailing zeros. If you had "1.10000", it would turn your number to "1.1". This functionality allows you to guarantee that the numbers are printed in a readable way.
